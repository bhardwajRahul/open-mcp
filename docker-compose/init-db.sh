...
# (e.g. "boltmcp_web") to avoid case-sensitivity issues.
...
psql --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" \
    -v ON_ERROR_STOP=1 \
    -v web_user="$BOLTMCP_INIT_WEB_USER" \
    -v web_pass="$BOLTMCP_INIT_WEB_PASSWORD" \
    ...
    <<-EOSQL
    -- Create web user (no schema needed - uses public schema)
    ...
    -- Grant default privileges for tables created by web user to mcp_server user
    -- We are assuming that it's the web user who runs migrations
    -- MCP server only gets SELECT (read-only) access to public tables
    ALTER DEFAULT PRIVILEGES FOR ROLE :"web_user" IN SCHEMA public GRANT SELECT ON TABLES TO :"mcp_user";
    ALTER DEFAULT PRIVILEGES FOR ROLE :"web_user" IN SCHEMA public GRANT SELECT ON SEQUENCES TO :"mcp_user";
    ...
    ALTER DEFAULT PRIVILEGES FOR ROLE :"web_user" IN SCHEMA public GRANT SELECT ON TABLES TO :"playground_user";
    ALTER DEFAULT PRIVILEGES FOR ROLE :"web_user" IN SCHEMA public GRANT SELECT ON SEQUENCES TO :"playground_user";
...
#    - Web & MCP Server: Use the public schema (PostgreSQL's default schema), so they share the same tables and can interact with the same data
