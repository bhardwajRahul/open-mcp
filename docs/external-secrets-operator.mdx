...
**Per-release setup** — populate the three Vault entries (`secret/boltmcp/database`, `secret/boltmcp/oidc`, `secret/boltmcp/auth`) with the key names listed in [Cluster Prep](/docs/cluster-preparation#application-secrets) (`secret/boltmcp/oidc` carries all three client secrets), then apply three `ExternalSecret` resources. Here's the database one as a worked example:
