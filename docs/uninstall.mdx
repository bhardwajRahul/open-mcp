...
<Callout type="info">
Depending on their delete policy, some Helm hook Jobs (`migrate-core`, `seed`, `keycloak-configure`) may survive `helm uninstall`. This only matters if you intend to keep the namespace — deleting the namespace (below) clears them.
</Callout>
