...
<Callout type="info">
The `ebs.csi.eks.amazonaws.com` provisioner is the one built into **EKS Auto Mode**. On a classic managed-node-group EKS cluster, install the [EBS CSI driver](https://github.com/kubernetes-sigs/aws-ebs-csi-driver) add-on instead and use its provisioner, `ebs.csi.aws.com`.
</Callout>
...
| `mcp-server-password` | DB password for the MCP server |
| `keycloak-password` | DB password for Keycloak |
| `vault-password` | DB password for Vault's storage backend (required when `vault.enabled=true`, the default) |
...
| `rest-api-resource-server-client-secret` | OIDC client secret used by the REST API to verify tokens. |
...
| `mcp-inspector-proxy-auth-token` | Proxy auth token |
