...
The OIDC client secrets in the `boltmcp-oidc` Secret must match what's configured on the corresponding clients in Keycloak. There are three client secrets: `web-client-secret`, `mcp-server-client-secret`, and `rest-api-resource-server-client-secret`. To rotate a value:
...
2. Update the same value on the matching client in the Keycloak admin console.
