...
| `mcp-client-client-secret` | OIDC client secret for external MCP clients (e.g. Claude Code) |
| `mcp-server-to-rest-api-client-secret` | OIDC client secret used by the MCP server to obtain user tokens for the REST API (PKCE-aware, audience-mapped to `REST_API_BASE_URL`). |
